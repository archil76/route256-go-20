...
func (r *Repository) updateOrder(ctx context.Context, order model.Order) (*model.Order, error) {
	...
	if _, err := r.pool.Exec(ctx, queryOrders, order.OrderID, order.UserID, order.Status); err != nil {
	...
	for _, item := range order.Items {
		if _, err := r.pool.Exec(ctx, queryOrderItems, order.OrderID, item.Sku, item.Count); err != nil {
			return nil, errors.Wrap(err, "pgx.QueryRow.Scan")
		}
